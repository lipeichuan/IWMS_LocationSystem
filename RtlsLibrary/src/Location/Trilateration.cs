...
namespace RtlsLibrary.src.Location
{
    ...
    public class Trilateration
    {
        ...
        //int  TRILATERATION                              =(1);
        //int REGRESSION_NUM                              =10;
        //double SPEED_OF_LIGHT                           =(299702547.0);   // in m/s in air
        //int NUM_ANCHORS                                 =(5);
        //int REF_ANCHOR                                  =(5);	//anchor IDs are 1,2,3,4,5 etc. (don't start from 0!)
        int TRIL_3SPHERES = 3;
        int TRIL_4SPHERES = 4;

        public unsafe static int GetLocation()
        {
        ...
        */
        unsafe int sphereline(vec3d p1, vec3d p2, vec3d sc, double r, double* mu1, double* mu2)
        {
            double a, b, c;
            double bb4ac;
            vec3d dp;
...
*/
        unsafe int trilateration(vec3d* result1,
                          vec3d* result2,
                          vec3d* best_solution,
                           vec3d p1, double r1,
                           vec3d p2, double r2,
        ...
        */
        unsafe int deca_3dlocate(vec3d* solution1,
                            vec3d* solution2,
                            vec3d[] best_solution,
        ...
        public unsafe int GetLocation(vec3d[] best_solution, int use4thAnchor, vec3d[] anchorArray, double[] distanceArray)
        {
            ...
            */
            ...
            if (result >= 0)
            {
                if (use4thAnchor == 1) //if have 4 ranging results, then use 4th anchor to pick solution closest to it
                {
                    ...
                    if (diff1 < diff2) best_solution[0] = o1; else best_solution[0] = o2;
                }
                else
                {
                    //assume tag is below the anchors (1, 2, and 3)
                    if (o1.z < p1.z) best_solution[0] = o1; else best_solution[0] = o2;
                }
            }
